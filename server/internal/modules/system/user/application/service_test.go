...
func TestCurrentFallsBackToActorWhenRepositoryUnavailable(t *testing.T) {
	service := NewService(&fakeRepository{err: domain.ErrRepositoryUnavailable}, fakeHasher{}, nil)
...
func TestCurrentMissingActor(t *testing.T) {
	_, err := NewService(nil, fakeHasher{}, nil).Current(context.Background())
...
func TestCurrentUserNotFound(t *testing.T) {
	_, err := NewService(&fakeRepository{err: domain.ErrUserNotFound}, fakeHasher{}, nil).Current(actorContext())
	var appErr *apperrors.Error
...
func TestChangePassword(t *testing.T) {
	...
	service := NewService(repo, fakeHasher{}, nil)
...
func TestChangePasswordWrongOldPassword(t *testing.T) {
	_, err := NewService(&fakeRepository{passwordHash: "hash:old-password"}, fakeHasher{}, nil).ChangePassword(actorContext(), ChangePasswordCommand{
...
func TestChangePasswordRepositoryUnavailable(t *testing.T) {
	_, err := NewService(nil, fakeHasher{}, nil).ChangePassword(actorContext(), ChangePasswordCommand{
		OldPassword: "old-password",
		NewPassword: "new-password",
	})
	var appErr *apperrors.Error
...
func TestUpdateProfileRepositoryUnavailable(t *testing.T) {
	_, err := NewService(nil, fakeHasher{}, nil).UpdateProfile(actorContext(), UpdateProfileCommand{NickName: "New Admin"})
	var appErr *apperrors.Error
	if !errors.As(err, &appErr) || appErr.Kind != apperrors.Internal {
		t.Fatalf("error = %v, want internal app error", err)
	}
...
func TestCreateUser(t *testing.T) {
	service := NewService(&fakeRepository{user: domain.User{ID: 2, UUID: "new", Username: "newuser", Enable: 1}}, fakeHasher{}, nil)
...
func TestCreateUserMissingActor(t *testing.T) {
	_, err := NewService(&fakeRepository{}, fakeHasher{}, nil).Create(context.Background(), CreateUserCommand{Username: "x", Password: "y"})
	var appErr *apperrors.Error
	if !errors.As(err, &appErr) || appErr.Kind != apperrors.Unauthorized {
...
func TestDeleteUser(t *testing.T) {
	got, err := NewService(&fakeRepository{}, fakeHasher{}, nil).Delete(actorContext(), 99)
	if err != nil {
...
func TestDeleteSelf(t *testing.T) {
	_, err := NewService(&fakeRepository{}, fakeHasher{}, nil).Delete(actorContext(), 1)
	var appErr *apperrors.Error
	if !errors.As(err, &appErr) || appErr.Kind != apperrors.Validation {
...
func TestResetPassword(t *testing.T) {
	...
	got, err := NewService(repo, fakeHasher{}, nil).ResetPassword(actorContext(), ResetPasswordCommand{
		UserID:   2,
		Password: "newpass",
	})
	if err != nil {
